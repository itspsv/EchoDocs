...
// rag_backend/server.py. Local only, started separately from this page.
const RAG_BACKEND = 'http://localhost:8001'
...
// --- client-side tools (retrieve_context) ---
// Whether a reply (e.g. a "let me check" transition phrase) is currently
// being spoken. A tool.result is only safe to send while this is false:
// true and it would land mid transition-phrase; a "hold" tool can also
// resolve with no transition phrase at all, in which case there is nothing
// to wait for and sending immediately is correct, not early.
let replyInFlight = false
const readyToolResults = []

function trySendToolResults() {
  if (replyInFlight || ws?.readyState !== 1) return
  while (readyToolResults.length) {
    const payload = readyToolResults.shift()
    ws.send(JSON.stringify(payload))
    logEvent('up', payload.type, payload.call_id)
  }
}

async function handleToolCall(msg) {
  if (msg.name !== 'retrieve_context') return
  const query = msg.arguments?.query ?? ''
  // tool.result has no top-level error field: type, call_id and a required,
  // always-a-string result are the only properties the schema accepts, so a
  // failure is just a different shape of that same JSON-encoded string.
  // https://www.assemblyai.com/docs/voice-agents/voice-agent-api/api-spec/voice-agent-websocket
  let content
  try {
    const res = await fetch(`${RAG_BACKEND}/retrieve`, {
      method: 'POST',
    ...
    content = data.chunks?.length
      ? { passages: data.chunks.map((c) => c.text) }
      : { error: 'No relevant passage found in the uploaded document for this query.' }
  } catch (err) {
    content = { error: `Could not reach the document backend: ${err.message}` }
  }
  const payload = { type: 'tool.result', call_id: msg.call_id, result: JSON.stringify(content) }
  readyToolResults.push(payload)
  trySendToolResults()
}
...
// --- document upload, feeds rag_backend/server.py ---
function setDocStatus(text, isError) {
  $('doc-status').textContent = text
  $('doc-status').classList.toggle('error', !!isError)
}

async function refreshDocStatus() {
  try {
    const res = await fetch(`${RAG_BACKEND}/status`)
    const data = await res.json()
    setDocStatus(data.loaded
      ? `Loaded: ${data.source} (${data.chunks} chunks)`
      : 'No document loaded.')
  } catch {
    setDocStatus('RAG backend not reachable — run: python rag_backend/server.py', true)
  }
}

async function loadDoc() {
  const fileInput = $('doc-file')
  const urlInput = $('doc-url')
  const file = fileInput.files[0]
  const url = urlInput.value.trim()
  if (!file && !url) {
    setDocStatus('Choose a file or paste a URL first.', true)
    return
  }
  $('doc-load').disabled = true
  setDocStatus('Loading…')
  try {
    const res = file
      ? await fetch(`${RAG_BACKEND}/ingest`, { method: 'POST', body: (() => {
          const form = new FormData()
          form.append('file', file)
          return form
        })() })
      : await fetch(`${RAG_BACKEND}/ingest`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    setDocStatus(`Loaded: ${data.source} (${data.chunks} chunks)`)
    fileInput.value = ''
    urlInput.value = ''
  } catch (err) {
    setDocStatus(`Error: ${err.message}`, true)
  } finally {
    $('doc-load').disabled = false
  }
}

$('doc-load').onclick = loadDoc
refreshDocStatus()
  ...
  try {
    ...
    ws.onmessage = ({ data }) => {
      ...
      switch (msg.type) {
        ...
        case 'tool.call': {
          // http tools run on AssemblyAI's side; retrieve_context has no
          // http block, so it's answered here instead. See handleToolCall.
